...
with DAG(dag_id="Sprin4_Task71", schedule_interval="@daily", default_args=default_args, catchup=False) as dag:
    ...
    with TaskGroup(group_id='group1') as fg1:
        ...
        f4 = FileSensor(task_id='waiting_for_file_price_log',
            fs_conn_id="fs_local", 
            filepath=str(datetime.now().date()) + "price_log.csv",
            poke_interval=5 ,
            on_success_callback = check_success_file_price_log,
            on_failure_callback = check_failure_file_price_log)
    ...
    with TaskGroup(group_id='group2') as fg2:
        ...
        load_price_log = PythonOperator(task_id='load_price_log', 
            python_callable=load_file_to_pg, 
            op_kwargs={'filename': str(datetime.now().date()) + "price_log.csv",
            'pg_table': 'staging.price_log'})
        
        sql_check = SQLCheckOperator(task_id="user_order_log_isNull", 
        ...
        sql_check2 = SQLCheckOperator(task_id="user_activity_log_isNull", 
        ...
        sql_check3 = SQLValueCheckOperator(task_id="check_row_count_user_order_log", 
        ...
        sql_check4 = SQLValueCheckOperator(task_id="check_row_count_user_activity_log", 
